...
# 1. MySQL 이란?
> MySQL은 전 세계에서 가장 널리 사용되는 오픈소스 관계형 데이터베이스 관리 시스템(RDBMS) 입니다.
데이터를 테이블(Table) 형태로 저장하고, 여러 테이블을 관계(Relationship) 로 연결해 구조적으로 관리할 수 있습니다.
SQL이라는 표준 언어를 사용해 데이터를 조회·추가·수정·삭제할 수 있습니다.
웹 서비스, 앱, ERP 등 거의 모든 소프트웨어에서 활용됩니다.
빠른 성능과 안정성, 무료 사용 가능하다는 장점 덕분에 교육용·실무용 모두에서 많이 사용됩니다.
